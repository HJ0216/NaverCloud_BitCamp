...
List<BoardDTO> listT = boardDAO.boardListT(map); // using map
List<BoardDTO> list = boardDAO.boardList(startNum, endNum); // using string
...
작성된 글이 여러개이므로 List<BoardDTO>에 담아오기
...
<!-- th: 표 제목, tr: 행, td: 열 -->
	...
	<% for(BoardDTO data : list){%>
		<tr>
			...
			<td><a class="subjectA" href="./boardView.jsp?seq=<%=data.getSeq() %>" onclick="return checkLogin()"><%=data.getSubject() %></a></td>
...
<% for(int i=1; i<=totalP; i++) {
		if(i==pg){%> 
			...
			<!-- currentPage일 경우, currentPagingDiv style 적용 및 클릭 시 해당 페이지로 이동 -->
	 <% } else { %>
			<div id='PagingDiv'><a id='paging' href="./boardList.jsp?pg=<%=i %>"> <%=i %> </a></div>
			<!-- currentPage가 아닐 경우, PagingDiv style 적용 및 클릭 시 해당 페이지로 이동 -->
	 <% } %>
<% } %>
...
<!-- style width: 1000px; div의 총 길이를 지정, align 기준이 화면 전체가 아닌 width가 기준이 됨 -->
